...
static int prepare_organization_section(char *input_files[],
    ...
    for (int i = 0; i < file_count; i++) {
        ...
        if (strchr(input_files[i], ',') ||
            strchr(input_files[i], '|')) {
        ...
        strncat(records_buffer,
                record,
                sizeof(records_buffer) - strlen(records_buffer) - 1);
    }
...
int extract_archive(const char *archive_name,
    ...
    if (fread(header_buffer,
              1,
              records_size,
              sau_file) != records_size) {
        ...
        if (sscanf(token,
            ...
            if (!out_file) {
                ...
                while (bytes_left > 0) {
                    ...
                    size_t chunk =
                        (bytes_left > (long)sizeof(data_buffer))
                        ? sizeof(data_buffer)
                        : (size_t)bytes_left;
                ...
                if (file_count > 0) {
                    ...
                    strncat(extracted_files_list,
                            ", ",
                            sizeof(extracted_files_list)
                            - strlen(extracted_files_list) - 1);
                }
